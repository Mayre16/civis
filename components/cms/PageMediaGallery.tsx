...
function GallerySlide({
  ...
  if (layout === "overlay") {
    return (
      <div
        className={`group relative aspect-[4/3] bg-na-civis/5 sm:aspect-[16/10] ${cardShell()} ${className}`}
      >
        {item.kind === "video" && src ? (
          <PageMediaVideo
            src={src}
  ...
  return (
    <div className={`${cardShell()} ${className}`}>
      <div className="relative aspect-[4/3] bg-na-civis/5 sm:aspect-[16/10]">
        {item.kind === "video" && src ? (
          <PageMediaVideo
            src={src}
            poster={poster}
      ...
      {(item.title || item.caption) && !hideCaption && (
        <div className="space-y-1 p-4">
          {item.title ? (
            <h3 className="text-base font-bold text-na-ink">{item.title}</h3>
          ) : null}
          {item.caption ? (
...
function GalleryMessage({
  ...
  const titleClass =
    variant === "slide"
      ? "text-balance text-xl font-black text-na-ink sm:text-2xl"
      : "text-balance text-2xl font-black text-na-ink sm:text-3xl";
...
function GalleryCarousel({
  ...
  if (n === 0) {
    return (
      <p className="rounded-2xl border border-dashed border-na-civis/20 bg-na-civis/5 px-4 py-10 text-center text-sm text-na-muted">
        ...
        <>
          <div className="pointer-events-none absolute inset-y-0 -left-2 -right-2 flex items-center justify-between sm:-left-3 sm:-right-3">
            <button
              type="button"
              onClick={goPrev}
              className="pointer-events-auto flex h-9 w-9 items-center justify-center rounded-full border border-na-civis/15 bg-white/95 text-na-civisDark shadow-sm transition hover:bg-white"
